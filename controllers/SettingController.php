...
class SettingController extends BackendController {
	...
	public function index($setting = '', $group = '') {
		...
		if (!$cfg) {
			return view(['error' => '不存在的配置'], 'setting/error');
		}
		if (!$cfg instanceof Setting) {
			return view(['error' => '配置不合法'], 'setting/error');
		}
		if (!$this->passport->cando($setting . ':system/setting')) {
			return view(['error' => '你没权限进行此项配置'], 'setting/error');
		}
		...
		$data                = [
			'group'   => $group,
			'setting' => $setting,
			'cfgurl'  => App::hash('~setting/' . $setting)
		];
		$data['groups']      = $groups;
		...
		if ($tpl) {//自定义模板
			...
			if (!$form instanceof FormTable) {
				return view(['error' => '无法识别的表单'], 'setting/error');
			}
			$data['rules'] = '';
	 ...
	 * @param string $setting
	 *
	 * @return \dashboard\classes\Setting
	 */
